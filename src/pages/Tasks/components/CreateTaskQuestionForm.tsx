...
import { z } from "zod"
import { useCreateQuestionMutation, useUpdateQuestionsMutation } from "@/hooks/tasks/mutations/useCreateQuestionMutation"
...
const questionItemSchema = z.object({
  ...
  if (val.type === "CHOICE") {
    ...
    if (nonEmptyOptions.length < 4) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "At least 4 options are required", path: ["option"] })
    }
...
const CreateTaskQuestionForm = ({ defaultTaskId, initialQuestion }: { defaultTaskId?: number; initialQuestion?: Partial<Question> }) => {
      ...
      <div className="space-y-6">
                  ...
                  {q.type === "WRITTEN" ? (
                    <div className="relative">
                      ...
                      <math-field
                        ...
                        className="w-full rounded-md border px-3 py-2 pr-10 text-base"
                        style={{ minHeight: 36 }}
                      />
                    </div>
                  ) : (
                    <Input
                      placeholder="Enter exact option text"
                      value={q.answer}
                  ...
                  <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                    {q.option.map((opt, optIdx) => (
                      <div key={optIdx} className="flex items-center gap-2">
                        ...
                        />
                        {q.option.length > 4 && (
                          <Button variant="ghost" onClick={() => removeOption(qIdx, optIdx)}>
                            −
                          </Button>
                        )}
                      </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" disabled={(mutation.isPending || updateMutation.isPending) || !canSubmit} onClick={handleSubmit}>
              {mutation.isPending || updateMutation.isPending ? (initialQuestion ? "Saving..." : "Creating...") : (initialQuestion ? "Save" : "Create")}
            </Button>
          </div>
        </div>
      </div>
