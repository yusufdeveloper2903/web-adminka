...
const fetchHereAutosuggest = async (params: HereAutosuggestParams): Promise<HereAutosuggestResponse> => {
  ...
  const response = await fetch(`https://autosuggest.search.hereapi.com/v1/autosuggest?${searchParams}`, {
    headers: {
...
export const useHereAutosuggestQuery = (params: HereAutosuggestParams, enabled: boolean = true) => {
